...
import { priceRanges, roomTypes, sortOtions } from '../../constants'


const CheckBox = ({label,selected = false, onchange = () => {}}) => {
    return (
        <label className='flex gap-3 items-center cursor-pointer mt-2 text-sm'>
            <input type="checkbox" checked={selected} onChange={(e) => (e.target.checked)}/>
            ...
            <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-gray-300 last:pb-30 last:border-0'>
                ...
                <div className='md:w-1/2 flex flex-col gap-2'>
                    ...
                    <div className='flex items-center'>
                        <StarRating />
                        <p className='ml-2 text-xl'>200+ reviews</p>
                    </div>
            ...
            <div className={`${openFilters ? 'h-auto' : 'h-0 lg:h-auto'} overflow-hidden transition-all duration-700`}>
                <div className='px-5 pt-5'>
                    ...
                    {roomTypes.map((room,index) => (
                        <CheckBox key={index} label={room.name}/>
                    ))}
                </div>
                <div className='px-5 pt-5'>
                    <p className='font-medium text-gray-800 pb-2'>Price Range</p>
                    {priceRanges.map((range,index) => (
                        <CheckBox key={index} label={range.name}/>
                    ))}
                </div>
                <div className='px-5 pt-5 pb-7'>
                    <p className='font-medium text-gray-800 pb-2'>Sort By</p>
                    {sortOtions.map((option,index) => (
                        <CheckBox key={index} label={option.name}/>
                    ))}
                </div>
            </div>
        </div>
